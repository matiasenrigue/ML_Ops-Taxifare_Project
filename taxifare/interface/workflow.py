...
"""
Code if I wanted to use Telegram:
1- Open Telegram
2- Search “BotFather”
3- Select menu at the bottom left corner and press ‘create new bot’
4- Go through instructions Telegram will provide
5- You will get an access token - copy it
...
def notify(old_mae, new_mae):
  ...
  if new_mae < old_mae and new_mae < 2.5:
    message = f":rocket: New model replacing old in production with MAE: {new_mae} the Old MAE was: {old_mae}"

  elif old_mae < 2.5:
    message = f":white_check_mark: Old model still good enough: Old MAE: {old_mae} - New MAE: {new_mae}"
  else:
    message = f":rotating_light: No model good enough: Old MAE: {old_mae} - New MAE: {new_mae}"
